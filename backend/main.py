    ...
    for item in items:
        ...
        if b_type in ["toggle", "to_do", "bulleted_list_item", "numbered_list_item", "quote", "callout"]:
            ...
            if isinstance(content, list):
                ...
                # 문자열인 경우 기존 방식
                rows_data = str(content).strip().split("\n") if content else []
            ...
            width = len(rows_data[0]) if rows_data and isinstance(rows_data[0], list) else (len(rows_data[0].split("|")) if rows_data else 1)
            base_block["table"] = {
                "table_width": width,
